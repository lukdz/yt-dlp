...
class BanByeIE(BanByeBaseIE):
    ...
    def _real_extract(self, url):
        ...
        protocol = 'https'
        match_obj = re.search(r'livestreamId:"(?P<id>[^"]+)"', webpage_content)
        if match_obj:
            m3u8_id = match_obj.groupdict().get('id')
            url_function = lambda quality: f'{self._TC2_BASE}/live/hls/{m3u8_id}_{quality}/index.m3u8'
            protocol = 'm3u8_native'
        else:
            match_obj = re.search(r'{video:{_id:"(?P<id>[^"]+)"', webpage_content)
            if match_obj:
                m3u8_id = match_obj.groupdict().get('id')
                url_tmp = lambda quality: f'{self._TC2_BASE}/edge/video/{m3u8_id}/v/index_{quality}/index.m3u8'
                url_check = url_tmp(data['quality'][0])
                try:
                    self._download_webpage(url_check, video_id, note="Checking " + url_check)
                except ExtractorError:
                    pass
                else:
                    url_function = url_tmp
                    protocol = 'm3u8_native'
